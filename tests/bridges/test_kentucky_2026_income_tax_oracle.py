...
MODULE = "us-ky:policies/income_tax/2026_full_year_resident_source_hold"
...
def test_kentucky_2026_exact_mappings_match_the_rulespec_output_set() -> None:
...
def test_kentucky_2026_bounded_outputs_use_exact_tax_classifications() -> None:
...
def test_kentucky_2026_bounded_outputs_remain_in_coverage(
    tmp_path: Path,
    program: str | None,
) -> None:
    mappings = _module_mappings()
    ...
    items = [
        item for item in report["items"] if item["legal_id"].startswith(f"{MODULE}#")
    ]
