...
DE_KINDERGELD_CLOSURE_BLOCKERS = [
    "closed: instrument frontier incomplete — 337 of 865 subordinate/bearing "
    "instruments pending disposition (oracles#491)",
    "closed: dependency closure open — 171 open dependencies (8 law-derived "
    "inputs, 0 unclassified inputs, 163 bearing instruments) (CERTIFIED.md v3)",
]
...
def test_de_certificate_exercise_is_measured_and_closure_is_source_scoped():
    ...
    assert closed["instrument_frontier"]["instrument_count"] == 865
    assert closed["dependency_closure"]["closed"] is False
    assert closed["dependency_closure"]["open_dependency_count"] == 171
