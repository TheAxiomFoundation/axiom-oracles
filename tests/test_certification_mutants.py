...
DE_KINDERGELD_CLOSURE_BLOCKERS = [
    "closed: instrument frontier incomplete — 268 of 781 subordinate/bearing "
    "instruments pending disposition (oracles#491)",
    "closed: dependency closure open — 156 open dependencies (8 law-derived "
    "inputs, 0 unclassified inputs, 148 bearing instruments) (CERTIFIED.md v3)",
]
...
def test_de_certificate_exercise_is_measured_and_closure_is_source_scoped():
    ...
    assert closed["instrument_frontier"]["instrument_count"] == 781
    assert closed["dependency_closure"]["closed"] is False
    assert closed["dependency_closure"]["open_dependency_count"] == 156
