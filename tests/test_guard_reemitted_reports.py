...
def test_unreadable_reports_are_not_reports(guard):
    """Invalid UTF-8 or JSON on either side never crashes the push loop, and
    is never mistaken for a real report to put back."""
    module, repo = guard
...
def test_restore_touches_only_the_downgraded_report(guard):
    ...
    module, repo = guard
    (repo / DATA / "r[e]al.json").write_text(_report("r[e]al", REAL))
    _git(repo, "add", "-A")
    _git(repo, "commit", "-q", "-m", "glob-named report")
    (repo / DATA / "r[e]al.json").write_text(_report("r[e]al", REEMITTED, stamp="t1"))
    neighbour = _report("real", REAL, stamp="t9")  # a real run over a real run
    (repo / DATA / "real.json").write_text(neighbour)

    assert module.main([]) == 0

    assert (repo / DATA / "real.json").read_text() == neighbour
    assert json.loads((repo / DATA / "r[e]al.json").read_text())["provenance"][
        "run_kind"
    ] == "manual"
