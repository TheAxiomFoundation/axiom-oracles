...
def test_wrong_engine_rejected_before_compilation(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        evidence.evidence_snapshot, "verify_git_commit", lambda *_args: None
    )
    monkeypatch.setattr(
        evidence.evidence_snapshot,
        "verify_engine_binding",
        lambda **_kwargs: {
            "engine_source_repository": evidence.evidence_snapshot.ENGINE_SOURCE_REPOSITORY,
            "engine_source_ref": evidence.evidence_snapshot.ENGINE_SOURCE_REF,
            "engine_sha256": evidence.evidence_snapshot.INPUT_INVENTORY_ENGINE_SHA256,
        },
    )
