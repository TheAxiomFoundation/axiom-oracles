...
try:
    from scripts import us_tariff_evidence_snapshot as evidence_snapshot
except ModuleNotFoundError:  # Direct execution: python scripts/<producer>.py
    import us_tariff_evidence_snapshot as evidence_snapshot
...
MODULES = (
    evidence_snapshot.WITNESS_MODULE,
...
ORIGINS = ("CN", "HK", "FR")
DAY = "2026-02-16"
# Source-linked reviewed transcription from note 2(u): absent
# donation/information/baggage, only a claim AND CBP agreement AND no 9802
# carve-out produces the exception. This is not machine-derived from the PDF.
...
def source_evidence(corpus: Path) -> dict:
    notes = git_bytes(corpus, evidence_snapshot.CORPUS_REF, evidence_snapshot.NOTES)
    require(
        digest(notes) == evidence_snapshot.NOTES_SHA256, "pinned notes digest drift"
    )
    pdf = git_bytes(corpus, evidence_snapshot.CORPUS_REF, PDF)
    require(
        digest(pdf) == PDF_SHA256 and pdf.startswith(b"%PDF-"),
        "pinned PDF digest/format drift",
    )
    manifest_bytes = git_bytes(corpus, evidence_snapshot.CORPUS_REF, INGEST)
    manifest = json.loads(manifest_bytes)
    applied = {row["path"]: row["sha256"] for row in manifest["applied_files"]}
    require(
        applied.get(PDF) == digest(pdf)
        and applied.get(evidence_snapshot.NOTES) == digest(notes),
    ...
    return {
        ...
        "ref": evidence_snapshot.CORPUS_REF,
        "notes": binding(evidence_snapshot.NOTES, notes),
        "pdf": binding(PDF, pdf),
...
def boundary_scopes() -> list[dict]:
    scopes = dict(evidence_snapshot.CANONICAL_INPUT_SCOPES)
    evidence = (
        (
            "176",
...
def execution_request(module: str, fixtures: list[dict]) -> dict:
    ...
    for row in fixtures:
        ...
        if module == evidence_snapshot.WITNESS_MODULE:
...
def build(
    *,
    ...
    rulespec: Path | None = None,
    engine_root: Path | None = None,
    engine: Path | None = None,
) -> dict:
    corpus = evidence_snapshot.corpus_root(corpus)
    rulespec = evidence_snapshot.rulespec_root(rulespec)
    evidence_snapshot.verify_git_commit(
        rulespec, evidence_snapshot.RULESPEC_REF, "RuleSpec"
    )
    engine_root = evidence_snapshot.engine_source_root(engine_root)
    engine = evidence_snapshot.engine_binary(engine, source_root=engine_root)
    engine_binding = evidence_snapshot.verify_engine_binding(
        source_root=engine_root, binary=engine
    )
    source = source_evidence(corpus)
    engine_bytes = engine.read_bytes()
    require(
        digest(engine_bytes) == evidence_snapshot.INPUT_INVENTORY_ENGINE_SHA256,
        "engine pin drift",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="tariff-boundary-evidence-") as raw:
        ...
        archive = subprocess.check_output(
            [
                ...
                str(rulespec),
                "archive",
                evidence_snapshot.RULESPEC_REF,
                "us",
                "programs",
            ]
        )
    ...
    return {
        ...
        "source": source,
        "rulespec_ref": evidence_snapshot.RULESPEC_REF,
        **engine_binding,
        "expectation_provenance": "source-linked reviewed transcription",
        ...
        "validation": {
            ...
            "query_execution_count": 384,
            "distinct_entity_id_count": 192,
            "checked_output_binding_count": 768,
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="replay all 384 query executions and compare exact receipt bytes",
    )
    ...
    try:
        ...
        return 1
    print(
        "boundary evidence reproduced: 6 scopes, 384 query executions / 192 "
        "distinct IDs, 768 output bindings; grounding=uncaptured, certified=false"
    )
    return 0
