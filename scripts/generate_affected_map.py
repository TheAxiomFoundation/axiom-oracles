...
the EUROMOD country). A SNAP QC replay (``snap-qc-compare``) uses none of the
three signals: it maps to exactly the one rulespec root its overlay is built
from (see ``snap_qc_repos``).
