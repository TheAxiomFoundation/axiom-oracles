...
leaving the weekly full matrix as the backstop (for every suite without
``ci:`` routing: a ``ci: manual`` suite refreshes only by a supervised run, and
a ``ci: <lane>`` suite is run by that lane's workflow).
