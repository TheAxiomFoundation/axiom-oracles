...
* ``committed_decisions`` are the reviewable, text-bound dispositions of
  spine provisions, candidate instruments, and frontier leaves (empty until
  discovery reads begin; every row binds the generated fact it decides);
* ``computed`` is a pure, fail-closed join.  Anything without a committed
  decision remains pending; ``closed`` computes true only when every spine
  row, candidate instrument, and boundary leaf is dispositioned and no
  law-derived leaf or bearing instrument is left unencoded.
...
#: Per-heading rows parsed from documents the snapshot retrieved
#: (scripts/parse_de_subject_documents.py). Optional: absent means no
#: document layer; present, it must bind the committed snapshot exactly.
HEADINGS_PATH = REPO_ROOT / "conformance" / "closure" / "de-subject-document-headings.json"
CLASS_DISCOVERY_PATH = (
    REPO_ROOT / "conformance" / "closure" / "de-discovery-2026-09-08" / "snapshot.json"
)

_HEADER = """# axiom_oracles.closure.ledger.v3 — GENERATED discovery ledger.
# generated_facts and computed are regenerated by scripts/de_closure_ledger.py.
# Edit only committed_decisions; every row must bind the generated fact it
# decides (spine body_sha256, candidate id, frontier input) and is validated
# against those facts on every --check.
"""
PROVISION_DECISION_STATUSES = (
    "encoded",
    "partially-encoded",
    "classified-with-reason",
    "excluded-with-reason",
)
INSTRUMENT_DECISION_STATUSES = (
    "encoded",
    "classified-with-reason",
    "excluded-with-reason",
)
...
def _snapshot_facts(
    ...
    headings_facts, heading_rows = _document_heading_candidates(
        program_id, snapshot, raw, snapshot_path, candidates
    )
    if heading_rows:
        candidates = sorted([*candidates, *heading_rows], key=lambda row: row["id"])
    class_facts, class_rows = _class_discovery_candidates(program_id, candidates)
    if class_rows:
        candidates = sorted([*candidates, *class_rows], key=lambda row: row["id"])
    facts = {
    ...
    if headings_facts is not None:
        facts["document_headings"] = headings_facts
    if class_facts is not None:
        facts["class_discovery"] = class_facts
    if "seed_bindings" in match:
        facts["seed_bindings"] = copy.deepcopy(match["seed_bindings"])
    return facts


def _class_discovery_candidates(
    program_id: str, candidates: Sequence[Mapping[str, Any]]
) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    """Import citation seeds, without asserting exhaustive class discovery.

    A discovery excerpt proves where a member was found, not its operative
    text. In particular, its hash must never become a candidate body hash.
    Existing class dispositions and all member dispositions remain separate.
    """
    if program_id != "de/kindergeld":
        return None, []
    path = CLASS_DISCOVERY_PATH
    # Unlike the older optional headings layer, this channel is required:
    # deleting its artifact must not silently shrink the frontier.
    try:
        raw = path.read_bytes()
        payload = _load_json_bytes(raw, str(path))
    except OSError as exc:
        raise _SourceError(f"cannot read class discovery snapshot: {exc}") from exc
    if (
        not isinstance(payload, Mapping)
        or payload.get("schema") != "axiom.de_kindergeld.discovery_snapshot.v1"
        or payload.get("complete") is not False
        or not isinstance(payload.get("remaining"), list)
        or not payload["remaining"]
    ):
        raise _SourceError("class discovery must declare its incomplete enumeration")
    receipt = _sha256(json.dumps(
        {key: value for key, value in payload.items() if key != "receipt_sha256"},
        ensure_ascii=False, sort_keys=True, separators=(",", ":"),
    ).encode())
    if payload.get("receipt_sha256") != receipt:
        raise _SourceError("class discovery receipt does not match snapshot bytes")
    sources = payload.get("sources")
    members = payload.get("members")
    if not isinstance(sources, list) or not sources or not isinstance(members, list):
        raise _SourceError("class discovery sources/members must be lists")
    source_text: dict[str, tuple[str, str]] = {}
    for source in sources:
        if not isinstance(source, Mapping) or not isinstance(source.get("path"), str):
            raise _SourceError("class discovery source path is malformed")
        name = source["path"]
        try:
            source_path = (path.parent / name).resolve()
            source_path.relative_to(path.parent.resolve())
            source_raw = source_path.read_bytes()
            normalized = " ".join(source_raw.decode("utf-8").split())
        except (OSError, ValueError) as exc:
            raise _SourceError(f"cannot verify class discovery source {name}: {exc}") from exc
        digest = _sha256(source_raw)
        if name in source_text or source.get("sha256") != digest:
            raise _SourceError("class discovery source is duplicated or has a stale hash")
        source_text[name] = (digest, normalized)
    known_ids = {row["id"] for row in candidates}
    counts = {f"de-kg-suppl-{number:03d}": 0 for number in range(7, 11)}
    rows = []
    for member in members:
        if not isinstance(member, Mapping):
            raise _SourceError("class discovery member must be a mapping")
        identity = member.get("identity")
        member_id = member.get("id")
        class_id = member.get("class_id")
        excerpt = member.get("discovery_excerpt")
        source_name = member.get("source_path")
        source = source_text.get(source_name) if isinstance(source_name, str) else None
        if (
            not isinstance(member_id, str)
            or not re.fullmatch(r"de-kg-discovery-[0-9a-f]{16}", member_id)
            or member_id in known_ids
            or not isinstance(identity, str) or not identity.strip()
            or not isinstance(class_id, str) or class_id not in counts
            or member.get("status") != "pending"
            or not isinstance(excerpt, str) or not excerpt.strip()
            or source is None
            or member.get("source_sha256") != source[0]
            or " ".join(excerpt.split()) not in source[1]
        ):
            raise _SourceError(f"invalid or unbound class discovery member: {member_id}")
        known_ids.add(member_id)
        counts[class_id] += 1
        rows.append({
            "id": member_id, "status": "pending",
            "identity_kind": "class_member", "identity": identity,
            "discovery_refs": [class_id],
            "discovery_source_path": (path.parent / member["source_path"]).relative_to(REPO_ROOT).as_posix(),
            "discovery_source_sha256": source[0], "discovery_excerpt": excerpt,
        })
    if payload.get("counts") != counts:
        raise _SourceError("class discovery counts do not match members")
    return ({
        "path": path.relative_to(REPO_ROOT).as_posix(), "sha256": _sha256(raw),
        "receipt_sha256": receipt, "complete": False, "counts": counts,
        "remaining": copy.deepcopy(payload["remaining"]),
    }, rows)


def _document_heading_candidates(
    program_id: str,
    ...
    """Frontier rows for every numbered heading of a retrieved subject document.

    The committed headings artifact is validated hermetically against the
    committed snapshot (bindings, receipts, identities) by the parser's pure
    validator; the PDF itself is never read here. Each row is a pending
    candidate of kind ``document_heading`` whose ``body_sha256`` is the located
    section text a disposition must bind.
    """

    headings_path = HEADINGS_PATH
    if not headings_path.is_file():
        return None, []
    raw = headings_path.read_bytes()
    payload = _load_json_bytes(raw, str(headings_path))
    parser_path = Path(__file__).resolve().parent / "parse_de_subject_documents.py"
    spec = importlib.util.spec_from_file_location("_de_ledger_subject_documents", parser_path)
    if spec is None or spec.loader is None:
        raise _SourceError("cannot load the subject-document parser validator")
    parser = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = parser
    spec.loader.exec_module(parser)
    try:
        parser.validate_artifact(
            payload,
            snapshot,
            snapshot_raw,
            snapshot_path.resolve().relative_to(REPO_ROOT).as_posix(),
        )
    except parser.ParseError as exc:
        raise _SourceError(f"subject-document headings artifact is invalid: {exc}") from exc
    known_ids = {row["id"] for row in candidates if isinstance(row, Mapping)}
    rows: list[dict[str, Any]] = []
    documents_summary: list[dict[str, Any]] = []
    for document in payload["documents"]:
        if document.get("program") != program_id:
            continue
        if document["seed_candidate_id"] not in known_ids:
            raise _SourceError(
                f"{document['subject_id']}: seed candidate {document['seed_candidate_id']} "
                "is not in the snapshot's candidate list"
            )
        documents_summary.append(
            {
                "subject_id": document["subject_id"],
                "seed_candidate_id": document["seed_candidate_id"],
                "content_sha256": document["content_sha256"],
                "heading_count": document["heading_count"],
                "sections_located": document["sections_located"],
            }
        )
        for heading in document["headings"]:
            if heading["id"] in known_ids:
                raise _SourceError(f"heading candidate id collides with a snapshot candidate: {heading['id']}")
            row = {
                "id": heading["id"],
                "status": "pending",
                "discovery_refs": [document["seed_candidate_id"], document["subject_id"]],
                "identity_kind": "document_heading",
                "document_subject_id": document["subject_id"],
                "document_sha256": document["content_sha256"],
                "heading_code": heading["code"],
                "title": heading["title"],
                "toc_page": heading["toc_page"],
                "heading_sha256": heading["heading_sha256"],
                "section_located": heading["section_located"],
            }
            if heading["section_located"]:
                row["body_sha256"] = heading["body_sha256"]
            rows.append(row)
    if not documents_summary:
        return None, []
    return (
        {
            "path": headings_path.relative_to(REPO_ROOT).as_posix(),
            "sha256": _sha256(raw),
...
def _canonical_decisions(
    ...
    *,
    ...
    """Validate committed decisions against the generated facts they bind and
    return them in canonical (sorted, stripped) form.

    Every decision binds the exact fact it decides: a provision by
    ``citation_path`` AND the spine row's ``body_sha256`` (text-grounded); an
    instrument by candidate ``id`` (and ``body_sha256`` when the candidate is
    a corpus row); a leaf by its frontier ``input``. Nothing a decision says
    can create a fact, and no fact can be decided twice.
    """
    ...
    if not isinstance(raw, Mapping):
        ...
        raw = {}
    if set(raw) - set(_DECISION_SECTIONS):
        errors.append("committed_decisions carries unknown sections")

    spine_by_citation = {
        row["citation_path"]: row for row in spine if isinstance(row, Mapping)
    }
    spine_order = {citation: index for index, citation in enumerate(spine_by_citation)}
    encoding_modules: dict[str, set[str]] = {}
    for module in modules:
        if (
            isinstance(module, Mapping)
            and module.get("state") == "captured"
            and isinstance(module.get("citation_path"), str)
            and isinstance(module.get("module_id"), str)
        ):
            encoding_modules.setdefault(module["citation_path"], set()).add(module["module_id"])
    all_module_ids = {mid for ids in encoding_modules.values() for mid in ids}

    provisions: list[dict[str, Any]] = []
    seen: set[str] = set()
    raw_rows = raw.get("provisions", [])
    ...
    for index, decision in enumerate(raw_rows):
        ...
        if not isinstance(citation, str) or citation not in spine_by_citation:
        ...
        if citation in seen:
            ...
            continue
        seen.add(citation)
        if decision.get("body_sha256") != spine_by_citation[citation].get("body_sha256"):
            errors.append(
                f"{label}.body_sha256 does not bind the spine row's provision text"
            )
        status = decision.get("status")
        if status not in PROVISION_DECISION_STATUSES:
            errors.append(f"{label}.status must be one of {PROVISION_DECISION_STATUSES}")
        row: dict[str, Any] = {
            "citation_path": citation,
            "body_sha256": decision.get("body_sha256"),
            "status": status,
        }
        ...
        if status in ("classified-with-reason", "excluded-with-reason"):
            ...
            row["reason"] = reason
        if status in ("encoded", "partially-encoded"):
            encoded_by = _text(decision.get("encoded_by"))
            if encoded_by is None or encoded_by not in encoding_modules.get(citation, set()):
                errors.append(
                    f"{label}.encoded_by must name a captured RuleSpec module bound "
                    f"to {citation}"
                )
            ...
            if status == "partially-encoded":
                if reason is None:
                    errors.append(
                        f"{label}.reason must state which part of the provision "
                        "remains unencoded"
                    )
                row["reason"] = reason
    ...
    provisions.sort(key=lambda row: spine_order[row["citation_path"]])

    candidate_by_id = {row["id"]: row for row in candidates if isinstance(row, Mapping)}
    candidate_order = {cid: index for index, cid in enumerate(candidate_by_id)}
    dispositions: list[dict[str, Any]] = []
    seen = set()
    raw_rows = raw.get("instrument_dispositions", [])
    if not isinstance(raw_rows, list):
        errors.append("committed_decisions.instrument_dispositions must be a list")
        raw_rows = []
    for index, decision in enumerate(raw_rows):
        label = f"committed_decisions.instrument_dispositions[{index}]"
        if not isinstance(decision, Mapping):
            errors.append(f"{label} must be a mapping")
            continue
        candidate_id = decision.get("id")
        if not isinstance(candidate_id, str) or candidate_id not in candidate_by_id:
            errors.append(f"{label}.id is not a generated instrument candidate")
            continue
        if candidate_id in seen:
            errors.append(f"duplicate instrument disposition: {candidate_id}")
            continue
        seen.add(candidate_id)
        candidate = candidate_by_id[candidate_id]
        status = decision.get("status")
        if status not in INSTRUMENT_DECISION_STATUSES:
            errors.append(f"{label}.status must be one of {INSTRUMENT_DECISION_STATUSES}")
        row = {"id": candidate_id, "status": status}
        bound_body = candidate.get("body_sha256")
        if isinstance(bound_body, str):
            if decision.get("body_sha256") != bound_body:
                errors.append(
                    f"{label}.body_sha256 does not bind the captured instrument text"
                )
            row["body_sha256"] = decision.get("body_sha256")
        elif decision.get("body_sha256") is not None:
            errors.append(
                f"{label}.body_sha256 names a text the candidate row does not capture"
            )
        classification = _text(decision.get("classification"))
        reason = _text(decision.get("reason"))
        if classification is None:
        ...
        if status in ("classified-with-reason", "excluded-with-reason"):
            if not isinstance(bears, bool):
                errors.append(
                    f"{label}.bears_on_computed_surface must be true or false: "
                    "does this instrument bear on any computed output (an open "
                    "dependency until encoded), or not"
                )
            row["bears_on_computed_surface"] = bears
        elif bears is not None:
            errors.append(
                f"{label}.bears_on_computed_surface is only valid for classified "
    ...
    seen = set()
    raw_rows = raw.get("leaf_classifications", [])
    if not isinstance(raw_rows, list):
        errors.append("committed_decisions.leaf_classifications must be a list")
        raw_rows = []
    for index, decision in enumerate(raw_rows):
        label = f"committed_decisions.leaf_classifications[{index}]"
        if not isinstance(decision, Mapping):
            errors.append(f"{label} must be a mapping")
            continue
        name = decision.get("input")
        if not isinstance(name, str) or name not in leaf_by_input:
            errors.append(f"{label}.input is not a generated frontier leaf")
            continue
        if name in seen:
            errors.append(f"duplicate leaf classification: {name}")
            continue
        seen.add(name)
        if leaf_by_input[name].get("leaf_kind") != "unclassified":
            errors.append(
                f"{label}: {name} is typed by closure/de/source.json and cannot be "
                "reclassified in the ledger"
            )
        kind = decision.get("leaf_kind")
        if kind not in LEAF_DECISION_KINDS:
            errors.append(f"{label}.leaf_kind must be one of {LEAF_DECISION_KINDS}")
        reason = _text(decision.get("reason"))
        if reason is None:
            errors.append(f"{label}.reason must be non-empty")
        row = {"input": name, "leaf_kind": kind, "reason": reason}
        defining = _text(decision.get("defining_citation_path"))
        if kind == "law_derived":
            if defining is None or not defining.startswith("de/"):
                errors.append(
                    f"{label}.defining_citation_path must name the DE provision that "
                    "defines how this input is computed"
                )
            row["defining_citation_path"] = defining
        elif defining is not None:
            errors.append(
                f"{label}.defining_citation_path is only valid for law_derived leaves"
            )
        classifications.append(row)
    classifications.sort(key=lambda row: row["input"])
...
def _empty_decisions() -> dict[str, Any]:
    return {section: [] for section in _DECISION_SECTIONS}
...
def _computed(
    ...
    """Pure join of generated facts and canonical committed decisions."""

    decisions = decisions or _empty_decisions()
    provision_decision = {
        row["citation_path"]: row for row in decisions.get("provisions", [])
    }
    ledger = []
    for row in spine:
        entry = {**copy.deepcopy(row), "status": "pending"}
        decision = provision_decision.get(row["citation_path"])
        if decision is not None:
            entry["status"] = decision["status"]
            for key in ("classification", "reason", "encoded_by"):
                if decision.get(key) is not None:
    ...
    for entry in ledger:
        provision_counts[entry["status"]] += 1
    pending_citations = [row["citation_path"] for row in ledger if row["status"] == "pending"]
    partially_encoded = [
        row["citation_path"] for row in ledger if row["status"] == "partially-encoded"
    ]

    leaf_decision = {row["input"]: row for row in decisions.get("leaf_classifications", [])}
    inputs = []
    for row in leaves:
        entry = copy.deepcopy(row)
        decision = leaf_decision.get(row["input"])
        if decision is not None:
            entry["leaf_kind"] = decision["leaf_kind"]
            entry["status"] = "classified"
            entry["reason"] = decision["reason"]
            if decision.get("defining_citation_path") is not None:
                entry["defining_citation_path"] = decision["defining_citation_path"]
        inputs.append(entry)
    law_derived = [row["input"] for row in inputs if row["leaf_kind"] == "law_derived"]
    unclassified = [row["input"] for row in inputs if row["leaf_kind"] == "unclassified"]
    pending_inputs = [row["input"] for row in inputs if row["status"] == "pending"]

    instrument_decision = {
        row["id"]: row for row in decisions.get("instrument_dispositions", [])
    }
    candidates = []
    for row in graph["candidates"]:
        entry = copy.deepcopy(row)
        decision = instrument_decision.get(row["id"])
        if decision is not None:
            entry["status"] = decision["status"]
    ...
    for entry in candidates:
        instrument_counts[entry["status"]] += 1
    pending_instruments = [row["id"] for row in candidates if row["status"] == "pending"]
    instruments_bearing = [
        row["id"]
        for row in candidates
        if row["status"] in ("classified-with-reason", "excluded-with-reason")
        and row.get("bears_on_computed_surface") is True
    ]
    reasons_complete = all(
        row["status"] in ("pending", "encoded")
        or (isinstance(row.get("reason"), str) and bool(row["reason"].strip()))
        for row in [*ledger, *candidates]
    )
    instrument_complete = bool(candidates) and not pending_instruments and reasons_complete
    boundary_complete = not unclassified
    open_dependencies = len(law_derived) + len(unclassified) + len(instruments_bearing)
    ...
    return {
        "ledger": ledger,
        "provision_counts": provision_counts,
        ...
        "dependency_closure": {
            ...
            "instruments_bearing_on_computed_count": len(instruments_bearing),
...
def generate_document(
    ...
    *,
    ...
    artifact_path = ARTIFACT_PATHS[program_id]
    if artifact_path.is_file():
        existing = load_document(artifact_path).get("committed_decisions")
        ...
        if decision_errors:
            raise ClosureLedgerError(
                [f"committed decision no longer binds the regenerated facts: {e}" for e in decision_errors]
            )
    computed = _computed(spine, leaves, graph, measurement_basis, decisions)
    return {
        "schema": SCHEMA,
        "program": {
            "id": program_id,
...
def _hermetic_rederivation(
    ...
    *,
    ...
    if decision_errors:
        raise ClosureLedgerError(decision_errors)
    expected["committed_decisions"] = decisions
...
def validate_artifact(document: Any) -> ClosureSummary:
    ...
    canonical = _canonical_decisions(
        ...
        leaves=leaves,
        candidates=candidates,
        modules=generated.get("rulespec_modules") or [],
        errors=errors,
    )
    if errors:
        raise ClosureLedgerError(errors)
    ...
    try:
        expected = _computed(spine, leaves, graph, measurement_basis, canonical)
    except (KeyError, TypeError) as exc:
        raise ClosureLedgerError((f"generated facts are malformed: {exc}",)) from exc
    ...
    if computed != expected:
        errors.append("computed block does not equal the pure derivation from facts and decisions")
    ...
    if not isinstance(computed.get("closed"), bool):
        errors.append("ledger closed must be a bool")
    dependency = computed.get("dependency_closure")
    if not isinstance(dependency, Mapping) or not isinstance(dependency.get("closed"), bool):
        errors.append("dependency closure closed must be a bool")
    boundary = computed.get("boundary_frontier")
    if not isinstance(boundary, Mapping) or not isinstance(boundary.get("complete"), bool):
        errors.append("boundary frontier complete must be a bool")
    if not isinstance(instrument_frontier, Mapping) or not isinstance(instrument_frontier.get("complete"), bool):
        errors.append("instrument frontier complete must be a bool")
    if any(
    ...
    return ClosureSummary(
        ...
        non_encoded_reasons_complete=expected["non_encoded_reasons_complete"] is True,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for program_id in programs:
            ...
            if result.valid:
                closed = bool(result.document["computed"]["closed"]) if result.document else False
                print(f"ok: {path.relative_to(REPO_ROOT)} (closed={str(closed).lower()})")
            else:
                failed = True
