...
def main() -> None:
    ...
    header = (
        (MODEL_ROOT / "Input" / "training_data.txt")
    ...
    for cid, rows in probes:
        ...
        print(
            f"  {cid}: WTC bwkmt_s={r.values.get('bwkmt_s')} "
    ...
    r = esa.run_cases(
        [case("disabled-single-no-ddipd", [dis])],
        variables=["bsadi_s", "bsadi01_s", "bsadi00_s"],
    )[0]
    print(
        f"  bsadi_s={r.values.get('bsadi_s')} "
