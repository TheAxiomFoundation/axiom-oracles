...
try:
    ...
    from scripts import us_tariff_live_source_evidence as sources
except ModuleNotFoundError:  # Direct execution: python scripts/<producer>.py
    import us_tariff_continuation_runtime as runtime
    import us_tariff_live_source_evidence as sources
...
WITNESS = runtime.original.evidence_snapshot.WITNESS_MODULE
MODULES = (
    WITNESS,
...
def capture(downloads):
    ...
    timestamp = datetime.fromtimestamp(
        (downloads / "china-2024-action.pdf").stat().st_mtime, UTC
    ).isoformat()
...
def source_evidence():
    live = sources.build(sources.DIRECTORY, sources.configured_corpus_root())
    sources.require(
...
def build():
    ...
    return {
        ...
        "rulespec_ref": runtime.original.evidence_snapshot.RULESPEC_REF,
        "engine_source_repository": runtime.original.evidence_snapshot.ENGINE_SOURCE_REPOSITORY,
        "engine_source_ref": runtime.original.evidence_snapshot.ENGINE_SOURCE_REF,
        "engine_sha256": runtime.original.evidence_snapshot.INPUT_INVENTORY_ENGINE_SHA256,
        ...
        "blocker": {
            ...
            "effect": "The real generated ch76/ch85 runtime returns zero with actual adapter facts, while the direct witness and stipulated reviewed source-membership inputs return the source rates for China. Hong Kong and France controls remain zero.",
