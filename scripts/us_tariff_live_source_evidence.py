...
try:
    ...
    import build_us_tariff_boundary_evidence as boundary
...
def configured_corpus_root(explicit: Path | None = None) -> Path:
    return boundary.evidence_snapshot.corpus_root(explicit)
...
def verify_signature(directory: Path, corpus: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="tariff-ingest-auth-") as temporary:
        ...
        issues = module.verify_ingest_manifest(
            ...
            public_key=key,
            repo=corpus,
            head_ref=boundary.evidence_snapshot.CORPUS_REF,
        )
    require(
        not issues,
    ...
    require(
        ...
        and applied[boundary.evidence_snapshot.NOTES]
    ...
    return {
        ...
        "canonical_verifier_corpus_ref": boundary.evidence_snapshot.CORPUS_REF,
...
def build(directory: Path, corpus: Path) -> dict:
    ...
    require(
        sha(notes.read_bytes()) == boundary.evidence_snapshot.NOTES_SHA256,
    ...
    return {
        ...
        "generated_by": "scripts/us_tariff_live_source_evidence.py",
...
def capture(downloads: Path, directory: Path, corpus: Path) -> None:
    ...
    for stem in DOWNLOADS:
        ...
        times[stem] = datetime.fromtimestamp(
            (downloads / (stem + ".pdf")).stat().st_mtime, UTC
        ).isoformat()
    ...
    for name, path in (
        ...
        (directory / name).write_bytes(
            boundary.git_bytes(corpus, boundary.evidence_snapshot.CORPUS_REF, path)
        )
...
def main() -> int:
    ...
    mode.add_argument(
        "--write",
        action="store_true",
        help="regenerate the receipt from the committed raw capture",
    )
    ...
    try:
        corpus = configured_corpus_root(args.corpus_root)
        if args.capture_from:
            capture(args.capture_from, args.directory, corpus)
        result = build(args.directory, corpus)
