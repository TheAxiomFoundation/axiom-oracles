...
def resolve_input_paths(preview_root: Path) -> dict[Path, Path]:
    """Map committed logical inputs to their selected physical locations."""

    return {
        logical_path: (
            preview_root / logical_path.name
            if logical_path in PREVIEW_INPUTS
            else logical_path
        )
        for logical_path in EXPECTED_HASHES
    }


def input_receipt(path: Path, *, logical_path: Path | None = None) -> dict[str, Any]:
    logical_path = logical_path or path
    require(path.is_file(), f"missing preview input: {path}")
    actual = sha256(path)
    require(
        actual == EXPECTED_HASHES[logical_path],
        f"preview input hash drift: {path}: {actual}",
    )
    return {
        "path": relative(logical_path),
...
def selected_rate_join(
    new_mismatch: Path = NEW_MISMATCH,
    selected_intervals: Path = SELECTED_INTERVALS,
) -> tuple[
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=PREVIEW,
        help=(
            "physical directory containing the five pinned preview-1311 inputs; "
            "receipt paths remain bound to their repo-local logical locations"
        ),
    )
...
def main() -> int:
    args = parse_args()
    input_paths = resolve_input_paths(args.preview_root.resolve())
    target_mismatch = input_paths[TARGET_MISMATCH]
    new_mismatch = input_paths[NEW_MISMATCH]
    old_receipt_path = input_paths[OLD_RECEIPT]
    taxonomy_receipt_path = input_paths[TAXONOMY_RECEIPT]
    unmapped_receipt_path = input_paths[UNMAPPED_RECEIPT]
    ...
    inputs = {
        relative(logical_path): input_receipt(
            input_path, logical_path=logical_path
        )
        for logical_path, input_path in input_paths.items()
    }
    old_receipt = json.loads(old_receipt_path.read_text())
    taxonomy_receipt = json.loads(taxonomy_receipt_path.read_text())
    unmapped_receipt = json.loads(unmapped_receipt_path.read_text())
    for name, receipt in (
    ...
    selected_rates, selected_join_audit = selected_rate_join(
        new_mismatch, input_paths[SELECTED_INTERVALS]
    )
    parser_pairs = {
