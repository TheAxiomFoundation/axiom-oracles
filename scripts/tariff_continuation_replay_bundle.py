#!/usr/bin/env python3
"""Package/replay exact recorded native outcomes with the pinned Axiom CLI.
...
The portable verifier does not recompute source-derived expectations, mismatch
relations, or legal interpretations.
"""
...
SCHEMA = "axiom_oracles.us_tariff_schedule.continuation_replay.v2"
...
def checked_response(
    run: dict, result: subprocess.CompletedProcess
) -> tuple[int, int, int]:
    """Return rows/bindings/errors when the exact recorded native outcome recurs."""
    ...
    return (
        ...
        sum(len(row["outputs"]) for row in response["results"]),
        0,
    )
...
def validate_bindings(bodies: dict[str, bytes]) -> dict[str, dict]:
    ...
    for name, producer in RECEIPTS.items():
        ...
        require(
            ...
            "receipt engine source pin mismatch",
        )
        require(
            evidence["rulespec_ref"] == RULESPEC_REF, "receipt RuleSpec pin mismatch"
        )
        require(
            evidence.get("expectation_provenance")
            == "source-linked reviewed transcription"
            and evidence.get("expectations_independently_pdf_derived") is False,
            "receipt expectation provenance drift",
        )
        require(
...
def verify(root: Path, expected_manifest_sha256: str) -> dict:
    ...
    recorded_mismatch_records = [
        row
        for receipt in evidence.values()
        for row in receipt.get("known_mismatches", [])
    ]
    mismatch_scenario_ids = {row["case_id"] for row in recorded_mismatch_records}
    require(
        len(recorded_mismatch_records) == 30 and len(mismatch_scenario_ids) == 19,
        "recorded mismatch population drift",
    )
    require(
    ...
    with tempfile.TemporaryDirectory(prefix="tariff-continuation-offline-") as raw:
        ...
        for name, receipt in evidence.items():
            count = {
                "query_executions": 0,
                "successful_result_rows": 0,
                "returned_output_bindings": 0,
                "native_missing_input_errors": 0,
            }
            receipt_entity_ids: set[str] = set()
            for run in runs(receipt):
                ...
                rows, bindings, errors = checked_response(run, result)
                queries = run["request"]["queries"]
                entity_ids = {query["entity_id"] for query in queries}
                receipt_entity_ids.update(entity_ids)
                all_entity_ids.update(entity_ids)
                count["query_executions"] += len(queries)
                count["successful_result_rows"] += rows
                count["returned_output_bindings"] += bindings
                count["native_missing_input_errors"] += errors
            counts[name] = {
                **count,
                "distinct_entity_ids": len(receipt_entity_ids),
            }
    totals = {
        key: sum(count[key] for count in counts.values())
        for key in (
            "query_executions",
            "successful_result_rows",
            "returned_output_bindings",
            "native_missing_input_errors",
        )
    }
    totals["distinct_entity_ids"] = len(all_entity_ids)
    require(
        totals
        == {
    ...
    return {
        ...
        "recorded_relation_consistency_verified": False,
        "source_derivation_verified": False,
        "legal_interpretation_verified": False,
        "engine_binary_reproducible_build_verified": False,
        "recorded_mismatch_records": len(recorded_mismatch_records),
        "distinct_recorded_mismatch_scenario_ids": len(mismatch_scenario_ids),
...
def build(destination: Path) -> dict:
    ...
    snapshot = runtime.original.evidence_snapshot
    ...
    snapshot.verify_git_commit(rulespec, RULESPEC_REF, "RuleSpec")
    files = {
        "verify.py": Path(__file__).read_bytes(),
        "axiom-rules-engine": engine.read_bytes(),
    }
    for name, producer in RECEIPTS.items():
    ...
    with runtime.compiled(
        modules,
        rulespec=rulespec,
        engine_root=engine_repository,
        engine=engine,
    ) as invoke:
...
The real pinned Axiom engine replays 843 query executions covering 453 distinct
entity IDs, 831 successful result rows, 1817 returned output bindings and 12
intentional native missing-input errors. This includes the original 384 query
executions (192 IDs) and 768 output bindings.

Receipts record 30 source/runtime mismatch records across 19 unique scenarios.
Expected values are source-linked reviewed transcriptions, not independently
PDF-derived outputs. This verifier reproduces only exact recorded native
outcomes. It does not recompute expected-from-source values, source relations,
adapter relations, mismatch classifications, or legal interpretations.
