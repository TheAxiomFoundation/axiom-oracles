...
The weekly comparison matrix and the 6-hourly affected rerun used to run the
SNAP QC suites on bare runners. Those runners have no engine binary and no QC
public-use file, so ``scripts/run_comparison.py`` re-emitted the committed
dashboard report (``provenance.reemitted_report: true``). Those legs stayed
green whether or not the replay still worked: the California replay stopped
compiling after rulespec-us#1176 (2026-07-30) and nothing failed. The suites
now declare ``ci: snap-qc-replay`` (pending ones ``ci: manual``), so neither
bare matrix runs them; the affected rerun calls this lane instead. The live
